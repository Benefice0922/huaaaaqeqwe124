...
async def run_kolesa(
    ...
    if api_manager:
        try:
            ...
            # Передаем в load_settings
            api_manager.load_settings(settings_data)
            
            # Проверяем селектор "Ссылка [Link]" для включения/выключения API
                ...
                while True:
                    ...
                    for ad in owner_ads:
                        ...
                        try:
                            ...
                            # ← ДОБАВЬТЕ ЭТУ СТРОКУ ЗДЕСЬ (сразу после my_custom_text)
